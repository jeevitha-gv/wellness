...
session_start();
    ...
    $query = "SELECT * FROM `wellness` WHERE (status='created' and risk='Medium') or (status='Reported' and risk='Medium') or (status='reinvestigate' and risk='Medium') ORDER BY id DESC";
  ...
  <td><?php echo $rows['infowellsec'];?></td>
   <td><?php echo $rows['createat'];?></td>
     ...
     <td><?php echo $rows['name'];?></td>
      ...
      <?php if($rows['risk']=="Medium")
      {

        ?>
       <td><button class="btn btn-danger" style="border-radius: 50px;height: 20px; padding: 1%;background-color: #F58711;" ><?php echo $rows['risk'];?></button></td>
     <?php }?>
      <td>
      <?php
    ...
    if($rows['status']=="Reported"){
     ...
     <?php } ?>
         ...
         <td><a href="case/testingindex.php?id=<?php echo $rows['infowellsec'];?>" class="btn btn-primary" style="background-color: #034ea2;">Counselor</a></td>
